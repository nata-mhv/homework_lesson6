...
def define_name(func, *arg: str) -> str:
    func_name = func.__name__.replace('_', ' ').title()
    required_args = ', '.join(arg)
    return f'{func_name} [{required_args}]'
...
def open_browser(browser_name):
    actual_result = define_name(open_browser, browser_name)
    assert actual_result == "Open Browser [Chrome]"


def go_to_companyname_homepage(page_url):
    actual_result = define_name(go_to_companyname_homepage, page_url)
    assert actual_result == "Go To Companyname Homepage [https://companyname.com]"


def find_registration_button_on_login_page(page_url, button_text):
    actual_result = define_name(find_registration_button_on_login_page, page_url, button_text)
